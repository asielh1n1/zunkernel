...
exports.ExecCommand=function(values){
    switch (values[0]) {
        ...
        case 'bundle:model:sync':{//Busca los models del bundle y los sincroniza con la base de datos
            if(!zun.existBundle(values[1]))return showError("Bundle "+values[1]+" not exist");
            var driver=eval('zun.'+values[1]+'.config.database.driver');
        ...
        case 'bundle:model:sync-force':{//Busca los models del bundle y los sincroniza con la base de datos, borra los datos
            if(!zun.existBundle(values[1]))return showError("Bundle "+values[1]+" not exist");
            var driver=eval('zun.'+values[1]+'.config.database.driver');
            if(!/sequelize:/.test(driver))  
                return zun.console("This command only works with the sequelize handler","error")  
        ...
        case 'bundle:restapi':{//Genera una api rest para el modelo especificado
            if(!zun.existBundle(values[1]))return showError("Bundle "+values[1]+" not exist");
            var bundledir=zun.basedir+"/bundles/"+values[1];
            var driver=eval('zun.'+values[1]+'.config.database.driver');
            if(/sequelize/.test(driver))
                driver='sequelize';
            if (!fs.existsSync(bundledir+'/model/'+values[2]+'.js'))
                zun.console("This model not exist in the bundle:"+values[1],'warning');
            ...
            var routingTemplate=zun.swig.renderFile(zun.basedir+'/node_modules/zunkernel/lib/template_routing.json',{model:values[2]})
            if(!fs.writeFileSync(routingDir + "/"+values[2]+".json", routingTemplate)){
                //Leo el config.json del bundle
                var config=JSON.parse( fs.readFileSync(bundledir+'/config/config.json', "utf-8") );
                if(config.router.indexOf('routing/'+values[2]+".json")==-1)
                    config.router.push('routing/'+values[2]+".json");
                 fs.writeFileSync(bundledir + "/config/config.json", JSON.stringify(config, null, "\t"));
                var controllerTemplate=zun.swig.renderFile(zun.basedir+'/node_modules/zunkernel/lib/template_controller.js',{bundle:values[1],model:values[2],driver:driver})
                if(!fs.writeFileSync(bundledir + "/controller/"+values[2]+".js", controllerTemplate))
                    zun.console("The RestApi was create successfully.");
            }else zun.console("Error to create apirest:"+values[1],'error');
        }break;
