...
function Command(){
    ...
    this.defaultCommand=function(){
        ...
        this.registerCommand('asset',[{name:'-b',text:'Bundle name',required:true}],'Copy the resource of the public bundle folder for the www and vice versa foolder.',function(param,values){
            if(!zun.existBundle(param['-b']))return zun.console("Bundle "+param['-b']+" not exist",'error');
        ...
        this.registerCommand('create-bundle',[{name:'-b',text:'Bundle name',required:true}],'Create new bundle',function(params,values){
            ...
            fs.mkdirSync(zun.basedir + "/www/"+name_bundle);
            fs.mkdirSync(zun.basedir + "/www/"+name_bundle+"/js");
            fs.mkdirSync(zun.basedir + "/www/"+name_bundle+"/css");
            var bootstrapCss=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/src/bootstrap.min.css');
            fs.writeFileSync(zun.basedir+'/www/'+name_bundle+'/css/bootstrap.min.css',bootstrapCss);
            var jqueryJs=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/src/jquery.slim.min.js');
            fs.writeFileSync(zun.basedir+'/www/'+name_bundle+'/js/jquery.slim.min.js',jqueryJs);
            var bootstrapJs=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/src/bootstrap.min.js');
            fs.writeFileSync(zun.basedir+'/www/'+name_bundle+'/js/bootstrap.min.js',bootstrapJs);
            var popperJS=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/src/popper.min.js');
            fs.writeFileSync(zun.basedir+'/www/'+name_bundle+'/js/popper.min.js',popperJS);
            ...
            var dataControler = "exports.index=function(req,res){\n\tres.send(zun." + name_bundle + ".render('home.html'));\n}";
            dataControler += "\n\nexports.contact=function(req,res){\n\tres.send(zun." + name_bundle + ".render('contact.html'));\n}";
            ...
            else config.bundles.push({ "name": name_bundle.toLowerCase(),"router":"/","main":"controller/main.js" });
            fs.writeFileSync(zun.basedir + "/config.json", JSON.stringify(config, null, "\t"));
            //Creo una vista por defecto
            var index=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/index.html',{bundle:name_bundle});
            fs.writeFileSync(dirbundle + "/view/index.html", index);
            var contact=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/contact.html',{bundle:name_bundle});
            contact=contact.replace('{{bundle}}',name_bundle);
            fs.writeFileSync(dirbundle + "/view/contact.html", contact);
            var home=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/home.html',{bundle:name_bundle});
            home=home.replace('{{bundle}}',name_bundle);
            fs.writeFileSync(dirbundle + "/view/home.html", home);
        ...
        this.registerCommand('restapi',[{name:'-b',text:'Bundle name',required:true},{name:'-m',text:'Model name',required:true}],'Create REST Full API.',function(params,values){
            if(!zun.existBundle(params['-b']))return zun.console("Bundle "+params['-b']+" not exist",'error');
            ...
            var routingTemplate=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/template_routing.json',{model:params['-m']})
            if(!fs.writeFileSync(routingDir + "/"+params['-m']+".json", routingTemplate)){
                ...
                var controllerTemplate=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/template_controller.js',{bundle:params['-b'],model:params['-m'],driver:driver})
        ...
        //Comando que permite crear una api rest
        this.registerCommand('create-crud',[{name:'-b',text:'Bundle name',required:true},{name:'-m',text:'Model name',required:true}],'Create CRUD by Model.',function(params,values){
            if(!zun.existBundle(params['-b']))return zun.console("Bundle "+params['-b']+" not exist",'error');
            var bundledir=zun.basedir+"/bundles/"+params['-b'];            
            var dirbundle = zun.basedir + "/bundles/" + params['-b'].toLowerCase();
            var driver=zun.config.database.driver
            if (!fs.existsSync(bundledir+'/model/'+params['-m']+'.js')){
                return zun.console("This model not exist in the bundle:"+params['-b'],'warning'); 
                process.exit();
            } 
            var routingDir=bundledir+'/config/routing';
            //Si no existe la carpeta routing en el bundle especificado, la creo
		    if (!fs.existsSync(routingDir))
                fs.mkdirSync(routingDir);
            //Copio el template del routing para esa carpeta
            var routingTemplate=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/crud_routing.json',{model:params['-m']})
            fs.writeFileSync(routingDir + "/"+params['-m']+".json", routingTemplate)
            //Leo el config.json del bundle
            var config=JSON.parse( fs.readFileSync(bundledir+'/config/config.json', "utf-8") );
            if(config.router.indexOf('routing/'+params['-m']+".json")==-1)
                config.router.push('routing/'+params['-m']+".json");
                fs.writeFileSync(bundledir + "/config/config.json", JSON.stringify(config, null, "\t"));
            var controllerTemplate=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/crud_controller.js',{bundle:params['-b'],model:params['-m'],driver:driver})
            fs.writeFileSync(bundledir + "/controller/"+params['-m']+".js", controllerTemplate)
            var attributes=[]
            switch(driver){
                case 'sequelize':{
                    var sequelizeAttr=zun.db.modelManager.getModel(params['-m']).attributes;
                    for(var i in sequelizeAttr){
                        var sequelizeType=sequelizeAttr[i].type;
                        var type='';
                        if((sequelizeType instanceof zun.sequelize.INTEGER) || (sequelizeType instanceof zun.sequelize.BIGINT)|| (sequelizeType instanceof zun.sequelize.FLOAT) || (sequelizeType instanceof zun.sequelize.REAL)|| (sequelizeType instanceof zun.sequelize.DOUBLE)|| (sequelizeType instanceof zun.sequelize.DECIMAL))type='number';
                        if((sequelizeType instanceof zun.sequelize.STRING) || (sequelizeType instanceof zun.sequelize.TEXT))type='text';
                        if((sequelizeType instanceof zun.sequelize.DATE) || (sequelizeType instanceof zun.sequelize.DATEONLY))type='date';
                        if((sequelizeType instanceof zun.sequelize.BOOLEAN))type='checkbox';                
                        attributes.push({name:i,type:type,required:(sequelizeAttr[i].allowNull)?false:true})
                    }
                }break;
                default:{
                    zun.console('Driver "'+driver+'" is not support.');
                    process.exit();
                }break;
            }
            var crud_list=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/templates/crud_list.html',"utf-8");
            var crud_new=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/templates/crud_new.html',"utf-8");
            var crud_edit=fs.readFileSync(zun.basedir+'/node_modules/zunkernel/templates/crud_edit.html',"utf-8");

            var myswig = new zun.swig.Swig({varControls: ['{$', '$}'],tagControls:['{!', '!}']});
            crud_list=myswig.render(crud_list,{ locals: { attributes: attributes,bundle:params['-b'],model:params['-m']}});
            crud_new=myswig.render(crud_new,{ locals: { attributes: attributes,bundle:params['-b'],model:params['-m']}});
            crud_edit=myswig.render(crud_edit,{ locals: { attributes: attributes,bundle:params['-b'],model:params['-m']}});
            if (!fs.existsSync(dirbundle + "/view/"+params['-m']))
                fs.mkdirSync(dirbundle + "/view/"+params['-m']);                
            fs.writeFileSync(dirbundle + "/view/"+params['-m']+"/list.html", crud_list);
            fs.writeFileSync(dirbundle + "/view/"+params['-m']+"/new.html", crud_new);
            fs.writeFileSync(dirbundle + "/view/"+params['-m']+"/edit.html", crud_edit);
            zun.console("The CRUD was create successfully.");
            zun.execCommand(["repository",'-b',params['-b'],'-m',params['-m']]);   
            process.exit();
        })
        ...
        this.registerCommand('repository',[{name:'-b',text:'Bundle name',required:true},{name:'-m',text:'Model name',required:true}],'Create pattern Repository.',function(params,values){
            ...
            //Copio el template del routing para esa carpeta
            var repositoryTemplate=zun.renderFile(zun.basedir+'/node_modules/zunkernel/templates/template_repository.js',{bundle:params['-b'],model:params['-m'],driver:driver})
